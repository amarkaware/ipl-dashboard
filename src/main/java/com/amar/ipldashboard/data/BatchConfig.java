...
public class BatchConfig {
    ...
    @Bean
    public FlatFileItemReader<MatcjInput> reader() {
        return new FlatFileItemReaderBuilder<MatcjInput>().name("MatchItemReader")
                .resource(new ClassPathResource("match-data.csv")).delimited().names(FIELD_NAMES)
                .fieldSetMapper(new BeanWrapperFieldSetMapper<MatcjInput>() {
                    {
                        setTargetType(MatcjInput.class);
                    }
                }).build();
    }

    @Bean
    public MatcjDataProcessor processor() {
        return new MatcjDataProcessor();
    }
    ...
    public Step step1(JdbcBatchItemWriter<Match> writer) {
        return stepBuilderFactory.get("step1").<MatcjInput, Match>chunk(10)
